...
class _TodoScreenState extends State<TodoScreen> with SingleTickerProviderStateMixin {
  ...
  TodoPriority _selectedPriority = TodoPriority.medium; // <--- შეცვლილია
  ...
  void _addOrEditTodo({TodoItem? existing}) {
    if (existing != null) {
      ...
      _selectedPriority = existing.priority; // <--- არსებული პრიორიტეტის დაყენება
    } else {
      _titleController.clear();
      _descController.clear();
      _groupController.clear();
      _selectedPriority = TodoPriority.medium; // <--- შეცვლილია
    }
            ...
            children: [
              ...
              DropdownButtonFormField<TodoPriority>(
                ...
                decoration: InputDecoration(
                  ...
                  labelText: 'პრიორიტეტი: ენერგიის დონე', // <--- ქართული ტექსტი
                  labelStyle: const TextStyle(color: Colors.white38, fontFamily: 'Orbitron'),
                ),
                items: TodoPriority.values.map((priority) {
                  return DropdownMenuItem(
                    value: priority,
                    child: Text(
                      priority.toGeorgianString, // <--- ქართული ტექსტი
                      style: TextStyle(color: priority.toColor, fontFamily: 'Orbitron'), // <--- ფერები
                    ),
                  );
                }).toList(),
                onChanged: (value) {
                  if (value != null) {
                    setState(() { // setState საჭიროა დიალოგშივე ცვლილების ასახვისთვის
              ...
              setState(() {
                if (existing == null) {
                  _todos.add(TodoItem(
                    ...
                    priority: _selectedPriority, // <--- პრიორიტეტის მინიჭება
                  ));
                } else {
                  ...
                  if (index != -1) {
                    _todos[index] = TodoItem(
                      ...
                      priority: _selectedPriority, // <--- პრიორიტეტის განახლება
                    );
                  }
                }
                _saveTodos();
